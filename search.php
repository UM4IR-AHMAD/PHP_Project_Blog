...
<?php
                    ...
                    $SearchTerm = mysqli_real_escape_string($dbConnection, $_GET['search']);

                    // ----------- Paggination database area ---------------
                    // getting number of records/rows
                    $sqlQuery = "SELECT COUNT(post_id) as totalNoOfRecords FROM post
                    ...
                    $NoOfPages = ceil($Record['totalNoOfRecords'] / $ShowRecordsPerPage );
                    ...
                    // ------ showing data area --------
                    $sqlQuery = "SELECT post_id, title, description, category, author, post_img, post_date, 
...
<div id="main-container">
        <main>
            ...
            <div class = "pagination-container">
                    <ul>
                        <?php
                            // ----------- Paggination button area ---------------
                            if($CurrentPage > 1) echo '<li><a href="search.php?search='.$SearchTerm.'&page='.($CurrentPage - 1).'">Prev</a></li>';                
                            for ($i=1; $i <= $NoOfPages ; $i++) {
                                ...
                                echo '<li><a  class ="'.$HighlightPageNumber.'"  href="search.php?search='.$SearchTerm.'&page='.$i.'">'.$i.'</a></li>';
                            }
                            if($CurrentPage < $NoOfPages) echo '<li><a href="search.php?search='.$SearchTerm.'&page='.($CurrentPage + 1).'">Next</a></li>';
                        ?>
                    </ul>
</div>
        </main>
